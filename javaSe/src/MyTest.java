...
public class MyTest {
    ...
    public static String getMonth(String gzYear, int month) {
        return getString01(gzYear, month, GZ_MONTH, GZ_YEAR);
    }
    static String getString01(String gzYear, int month, String[][] gzMonth, String[] gzYear2) {
        String gzYearStr = gzYear.substring(0, 1);
        String resultStr = "";
        for (int i = 0; i < GZ_MONTH.length; i++) {
            if (gzYear2[i].contains(gzYearStr)) {
                resultStr = GZ_MONTH[i][month - 1];
            }
        }
        return resultStr;
    }
    ...
    public static String getDay(int year, int month, int day) {
        ...
        int iGan = (4 * C + C / 4 + 5 * y + y / 4 + 3 * M / 5 + d - 3) % 10;
        int iZhi = (8 * C + C / 4 + 5 * y + y / 4 + 3 * M / 5 + d + 7 + i) % 12;
